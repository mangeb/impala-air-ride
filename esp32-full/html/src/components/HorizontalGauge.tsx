...
export const HorizontalGauge: React.FC<HorizontalGaugeProps> = ({ 
  ...
  return (
    <div className="relative w-full h-20 bg-[#fdfbf7] rounded-lg border-2 border-black/40 shadow-inner overflow-hidden flex flex-col justify-end p-2">
      {/* Background Markings */}
      <div className="absolute inset-0 flex items-end px-4 pb-6 opacity-30">
      ...
      {/* Numbers */}
      <div className="absolute inset-x-0 top-0 flex justify-between items-start px-4 pt-2 opacity-60">
        {[0, 50, 100, 150].map((num) => (
          <span key={num} className="text-[10px] font-mono font-black text-black">{num}</span>
        ))}
      </div>

      {/* Target Indicator (Major Needle) */}
      {targetPercentage !== null && (
        <div className="absolute inset-y-0 left-4 right-4 pointer-events-none z-10">
          <motion.div 
      ...
      <div className="absolute inset-y-0 left-4 right-4 pointer-events-none z-0">
        <motion.div 
